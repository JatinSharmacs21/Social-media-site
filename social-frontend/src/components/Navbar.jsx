...
function Navbar() {
  ...
  const openBlockedList = async () => {
    ...
    try {
      ...
      logger.error("Failed to load blocked users:", error.message);
    } finally {
  ...
  const BlockedUsersModal = () => {
    if (!blockedListOpen) return null;
    ...
    return (
      <div className="fixed inset-0 z-[110] flex items-end justify-center bg-black/60 backdrop-blur-sm sm:items-center">
        ...
        <div className="relative w-full max-w-md rounded-t-3xl border border-white/10 bg-[#0c0b10] p-4 shadow-2xl sm:rounded-3xl sm:p-5">
          <div className="mb-3 flex items-center justify-between">
            <h3 className="text-base font-black text-white">Blocked users</h3>
            <button
              onClick={() => setBlockedListOpen(false)}
              className="rounded-full border border-white/10 bg-white/[0.06] px-3 py-1 text-xs font-bold text-slate-300 hover:bg-white/[0.1]"
            >
              Close
            </button>
          </div>
              ...
              blockedUsersList.map((blockedUser) => (
                ...
                >
                  <div className="flex min-w-0 items-center gap-2.5">
                    <img
                      src={blockedUser.profilePic || "/default-avatar.png"}
                      alt=""
                  ...
                  <button
                    ...
                    className="shrink-0 rounded-full border border-white/10 bg-white/[0.08] px-3 py-1.5 text-xs font-black text-white transition hover:bg-white/[0.14] disabled:opacity-50"
                  >
                    {unblockingId === blockedUser._id ? "..." : "Unblock"}
                  </button>
                </div>
              ))
            )}
          ...
          <div className="relative flex h-full min-h-0 flex-col overflow-hidden">
            ...
            <div className="vybe-scrollless min-h-0 flex-1 overflow-y-auto px-4 pb-6 pt-2 sm:px-5">
              ...
              <div className="space-y-2 overflow-hidden">
                ...
                <PanelItem title="Blocked users" subtitle="View and unblock people you've blocked" onClick={openBlockedList} />
