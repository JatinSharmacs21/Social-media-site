...
function WhisperMessageBubble({ message, mine, activePerson }) {
  return (
    <div className={`flex ${mine ? "justify-end" : "justify-start"}`}>
      <div className={`flex max-w-[86%] gap-2 md:max-w-[70%] ${mine ? "flex-row-reverse" : "flex-row"}`}>
        {!mine && (
        ...
        <div
          className={`group relative px-4 py-3 shadow-lg transition md:max-w-[620px] ${
            mine
              ? "rounded-[25px] rounded-br-[10px] bg-gradient-to-br from-pink-500 via-purple-500 to-cyan-500 text-white shadow-pink-500/15"
              : "rounded-[25px] rounded-bl-[10px] border border-white/10 bg-white/[0.075] text-zinc-100 shadow-black/25 backdrop-blur-xl"
          }`}
        >
