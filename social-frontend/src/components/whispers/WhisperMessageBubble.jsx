...
function ReplySnippet({ replyTo, mine, onJumpToMessage }) {
  ...
  return (
    ...
    >
      <p className={`text-[10px] font-bold uppercase tracking-[0.14em] ${mine ? "text-white/62" : "text-zinc-500"}`}>
        Reply
      </p>
      <p className={`mt-0.5 truncate text-xs font-medium ${mine ? "text-white/82" : "text-zinc-300"}`}>
        {replyTo.text}
      </p>
    </div>
  );
...
function DesktopMenu({ mine, message, deleting, onClose, onReplyToMessage, onDeleteMessage }) {
  ...
  return (
    <div
      className={`absolute bottom-9 z-40 hidden w-44 overflow-hidden rounded-2xl border border-white/[0.09] bg-[#0a0b10]/98 p-1.5 shadow-2xl shadow-black/65 backdrop-blur-2xl sm:block ${
        mine ? "right-0" : "left-0"
      }`}
    >
      <button
...
function WhisperMessageBubble({
  ...
  return (
    <div
      ...
      className={`group flex scroll-mt-24 transition ${mine ? "justify-end" : "justify-start"} ${
        ...
        <div className={`relative flex flex-col ${mine ? "items-end" : "items-start"}`}>
          ...
          >
            <div className="px-3.5 py-2.5 md:max-w-[620px] md:px-4 md:py-2.5">
              ...
              <p className="whitespace-pre-wrap break-words text-[14.5px] font-normal leading-relaxed text-white/95 md:text-[15px]">
                {message.text}
              </p>
              {isLastInGroup && (
            ...
            {menuOpen && (
              <>
                <button
                  type="button"
                  aria-label="Close message options"
                  className="fixed inset-0 z-30 hidden cursor-default bg-transparent sm:block"
